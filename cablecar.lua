...
local CABLE_CARS = {
    [0] = {
        ...
        speed = 17.5, 			-- 17,5 Original
		maxSpeedDistance = 50, 	-- 50 Original
        state = "IDLE",
		showTramBlips = true,
...
function GivePlayerOptionToJoinMyCablecar(cablecar, moving)
    ...
    if not cablecar.is_player_seated then
        ...
        local dist = #(pos - plypos)
        if dist < 3.0 then
            DrawCablecarText3D("Drücke ~g~E ~w~zum Einsteigen", pos.x, pos.y, pos.z + 1.0)
        ...
        if not moving then
            DrawCablecarText3D("Drücke ~g~E ~w~zum Aussteigen", pos.x, pos.y, pos.z + 1.0)
            if IsControlJustPressed(0, 38) then
                cablecar.is_player_seated = false
                DetachEntity(ply, 0, 0)
            end
